...
class get_data():
    ...
    def get_dttot(self):
        try:
            ...
            self.failed_list.append("download_DTTOT and  download_WMD failed..")


    def get_opec(self):
        try:
            print("Scrapping for OPEC...")
            text = []
            ...
            df = df.iloc[:23888]
            df["source"] = "OPEC"
            df.to_csv("./raw_data/OPEC_list.csv", index=False)
            print("Done")
        except Exception as e:
            self.error_desc.append(e)
    ...
    def get_uk(self):
        try:
            print("Scrapping for UK...")
            soup = self.get_request(self.url_uk)
            df = self.download_uk(soup)
            df.to_csv("./raw_data/UK_list.csv", index=False)
        except Exception as e:
            self.error_desc.append(e)
    ...
    def get_un(self):
        try:
            ...
            df.to_csv("./raw_data/UN_list.csv", index=False)
        except Exception as e:
            self.error_desc.append(e)
            self.failed_list.append("download_uk failed..")
    ...
    def get_data(self):
        ...
        self.get_uk()
        self.get_un()
        print("creating log_scrapping.xlsx file")
